...
console.log('Коля!');
Коля - так как интервал вызова функции showName задан 0, поэтому сначала выведется вызов в консоль значения 'Коля'
...
верно - метод объявления функции позволяет вызвать ее из любого места
// Задача №3
// Верно ли вызвана функция
showMessage();
...
неверное - метод функционального выражения позволяет вызвать функцию только после ее создания
// Задача №4
// Как решить проблему?
'use strict'
if (2 > 1) {
...
'use strict'
let showMessage; // создаем переменную

if (2 > 1) {
	showMessage = function() {		// присваиваем переменной функционально выражение
		console.log('Сообщение');
	}
}
showMessage(); // получаем корректный вывод в консоль
